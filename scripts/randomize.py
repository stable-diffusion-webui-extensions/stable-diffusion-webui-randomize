...
class RandomizeScript(scripts.Script):
    ...
    def _random_checkpoint(self, checkpoints):
        saved_checkpoint_file = Path(__file__).parent.joinpath(".checkpoint")
        checkpoint = random.choice(checkpoints)
        saved_checkpoint = None
        if saved_checkpoint_file.exists():
            with open(saved_checkpoint_file, "r") as file:
                saved_checkpoint = file.read()
        if isinstance(checkpoints, list):
            checkpoint_name = checkpoint
        else:
            checkpoint_name = checkpoint.name
        while saved_checkpoint and saved_checkpoint == checkpoint_name:
            print("Randomizing checkpoint.", "\n")
            checkpoint = random.choice(checkpoints)
            if isinstance(checkpoints, list):
                checkpoint_name = checkpoint
            else:
                checkpoint_name = checkpoint.name
        with open(saved_checkpoint_file, "w") as file:
    ...
    def ui(self, is_img2img):
        (
            randomize_enabled,
            randomize_param_sampler_name,
            randomize_param_cfg_scale,
            randomize_param_steps,
            randomize_param_width,
            randomize_param_height,
            randomize_hires,
            randomize_hires_denoising_strength,
            randomize_hires_width,
            randomize_hires_height,
            randomize_other_CLIP_stop_at_last_layers,
        ...
        **kwargs,
        ...
        if randomize_enabled and isinstance(p, StableDiffusionProcessingTxt2Img):
            ...
            all_opts = {
                ...
                if k not in ["self", "p", "randomize_enabled", "batch_number", "prompts", "seeds", "subseeds"]
            }
                    ...
                    if opt is not None:
                        if param in ["seed"]:
                            setattr(
                                p,
                                "all_seeds",
                                [self._opt({param: val}, p) for _ in range(0, len(getattr(p, "all_seeds")))],
                            )  # NOTE (mmaker): Is this correct?
                        else:
                            setattr(p, param, opt)
                    else:
    ...
    def _opt(self, opt, p):
        ...
        if self._is_num(opt_arr[0]) and len(opt_arr) == 3 and opt_name not in ["seed"]:
            ...
            if opt_name == "sd_model_checkpoint":
                if opt_val == "*":
                    return self._random_checkpoint(list(checkpoints_list.values()))
                return sd_models.get_closet_checkpoint_match(self._random_checkpoint(opt_arr))
    ...
    def _apply_styles(self, p: StableDiffusionProcessing):
        ...
        if type(p.negative_prompt) == list:
            p.all_negative_prompts = [
                shared.prompt_styles.apply_negative_styles_to_prompt(x, p.styles) for x in p.negative_prompt
            ]
        else:
            p.all_negative_prompts = (
                p.batch_size
    ...
    def _create_ui(self):
        ...
        with gr.Group():
            with gr.Accordion("Randomize", open=False):
                ...
                randomize_hires_denoising_strength = gr.Textbox(
                    label="Highres. Denoising Strength", value="", placeholder=hint_minmax
                )
                ...
                randomize_other_CLIP_stop_at_last_layers = gr.Textbox(
                    label="Stop at CLIP layers", value="", placeholder=hint_minmax
                )
                randomize_other_sd_model_checkpoint = gr.Textbox(
                    label="Checkpoint name", value="", placeholder=hint_list
                )
                ...
                randomize_other_eta_noise_seed_delta = gr.Textbox(
                    label="Eta noise seed delta", value="", placeholder=hint_minmax
                )
